...
plotPathwayHeat <- function(
    ...
    involved <- guessInvolvementPathway(
        pathway, min_prop_pca = discr_prop_pca,
        min_prop_events = discr_prop_events,
        ...
    )

    if (length(paletteNames) !=
        length(involved)) {
    ...
    ann_col <- lapply(
        colnames(annotationFull),
        function(name) {
            omic <- guessOmic(name)
            if (!omic %in%
                names(annotationPalettes)) {
            ...
            if (length(values) ==
                1) {
                annot <- as.character(
                  MOSpaletteSchema[discreteColor,
                    c("smart")]
              )
                names(annot) <- values
    ...
    if (!is.null(additionalAnnotations)) {
        ...
        if (!is.null(additionalPaletteNames)) {
            add_ann_col <- lapply(
                colnames(additionalAnnotations),
                function(name) {
    ...
    annotationFull <- sortAnnotations(
        annotationFull,
        sortBy
    )
    annotationFull <- annotationFull[,
        rev(colnames(annotationFull))]
    ...
    for (n in seq_along(involved)) {
        ...
        heatMatrix <- heatMatrix[,
            row.names(annotationFull),
            drop = FALSE]
        row.names(heatMatrix) <- conversionToSymbols(
            row.names(heatMatrix),
            orgDbi
        )
        Ht <- Heatmap(
            heatMatrix,
...
plotPathwayKM <- function(
    pathway, formula = "Surv(days, status) ~ PC1",
    fileName = NULL, paletteNames = NULL, h = 9,
    w = 7, risk.table = TRUE, pval = TRUE, size = 1,
    ...
    daysAndStatus <- coxObj[, c("status", "days"),
        drop = FALSE]
    ...
    coxObj <- data.frame(
        daysAndStatus, annotationFull[row.names(daysAndStatus),
            , drop = FALSE]
    )
    ...
    p <- survminer::ggsurvplot(
        fit, data = coxObj, risk.table = risk.table,
        pval = pval, palette = palette, size = size
    )

    if (!is.null(fileName)) {
        ggplot2::ggsave(
            filename = fileName, p, height = h,
            width = w
        )
    } else {
        p
...
plotModuleHeat <- function(
    moduleobj, moduleNumber,
    sortBy = NULL, paletteNames = NULL,
    additionalAnnotations = NULL,
    additionalPaletteNames = NULL,
    withSampleNames = TRUE,
    fontsize_row = 10,
    ...
    involved <- guessInvolvement(
        moduleobj, moduleNumber = moduleNumber,
...
plotModuleKM <- function(
    ...
    h = 9, w = 7, risk.table = TRUE,
    pval = TRUE, size = 1, inYears = FALSE,
    ...
    daysAndStatus <- coxObj[,
        days_status_names, drop = FALSE]
    if (inYears)
        daysAndStatus$days <- daysAndStatus$days/365.24
    ...
    if (!is.null(additional_discrete)) {
        ...
        if (length(not_found) >
            ...
            stop(
                paste0(
                  "Some discrete variables were not found: ",
                  paste(
                    not_found,
                    collapse = ", ",
                    "\n"
    ...
    if (!is.null(additional_continuous)) {
        ...
        if (length(not_found) >
            ...
            stop(
                paste0(
                  "Some continuous variables were not found: ",
                  paste(
                    not_found,
                    collapse = ", ",
                    "\n"
        ...
        df <- cbind(
            daysAndStatus, coxObj[row.names(daysAndStatus),
                fixed_covs, drop = FALSE]
        )
        ...
        if (!is.null(additional_clinic)) {
            ...
            additional_clinic <- discretized_covs[,
                fixed_covs, drop = FALSE]
        }
    }
    ...
    p <- survminer::ggsurvplot(
        fit, data = coxObj, risk.table = risk.table,
        pval = pval, palette = unname(palette),
        size = size
    )
...
plotModuleInGraph <- function(
    ...
    fileName = NULL, discr_prop_pca = 0.15, discr_prop_events = 0.05,
    pathTitle = NULL, ...
) {
    ...
    color[names(V(net)) %in%
        moduleGenes] <- "tomato"

    involved <- guessInvolvement(
        modulesobj, moduleNumber = moduleNumber,
        min_prop_pca = discr_prop_events, min_prop_events = discr_prop_events,
        ...
    )
    mark.groups <- lapply(
        involved, function(x) {
    ...
    if (!is.null(paletteNames)) {
        ...
        mark.col <- MOSpaletteSchema[paletteNames,
            ]$transparent
    }
    ...
    plot(
        net, edge.arrow.size = 0.5, edge.curved = 0.2,
        vertex.label = labels, vertex.label.cex = 0.6,
        vertex.label.family = "sans", vertex.label.font = 2,
        vertex.color = color, vertex.frame.color = "gray",
        vertex.label.color = "black", vertex.size = 15,
        mark.groups = mark.groups, mark.col = mark.col,
        mark.border = NA
    )
    legend(
        x = -1, y = -1, legendLabels, pch = 21,
        horiz = TRUE, col = "#777777", pt.bg = mark.col,
...
plotMultiPathwayReport <- function(
    ...
    cell_text <- function(j, i, x, y, width, height, fill) {
        grid.text(
            sprintf("%.2f", msummary[i, j]),
            x, y, gp = gpar(fontsize = fontsize)
        )
    }
...
plotModuleReport <- function(
    modulesObj, MOcolors = NULL, priority_to = NULL,
    fontsize = 12, ...
) {
    ...
    ht1 <- Heatmap(
        matrix = summary$pvalue, col = pvalueShades,
        cluster_rows = FALSE, show_heatmap_legend = FALSE,
        name = "Pvalue", column_names_gp = gpar(fontsize = fontsize),
        cell_fun = function(j, i, x, y, width, height, fill) {
            grid.text(
                sprintf("%.2f", summary$pvalue[i]),
                x, y, gp = gpar(fontsize = fontsize)
    ...
    defargs <- list(
        ...
        cell_fun = cell_text, top_annotation = ta,
