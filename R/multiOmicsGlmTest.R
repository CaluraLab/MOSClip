...
multiOmicsTwoClassPathwayTest <- function(
    ...
    useTheseGenes = NULL, nullModel = "classes ~ 1", pathName = NULL
) {
    ...
    } else if (length(baseFormula_input) ==
        1 | baseFormula_input[2] != "~") {
    ...
    if (!(nullModel_input[1] %in% colnames(classAnnot))) {
        stop("Invalid formula. Class column not found in classAnnot")
    } else if (length(nullModel_input) ==
        1) {
    ...
    new(
        "MultiOmicsPathway", pvalue = res$pvalue, zlist = res$zlist,
        pathView = moduleView,
...
multiOmicsTwoClassModuleTest <- function(
    ...
    } else if (nullModel_input[2] != "~" | nullModel_input[3] !=
        "1") {
    ...
    if (length(genes) ==
        0)
        stop(
            "There is no intersection between expression feature names and ",
    ...
    results <- lapply(
        cliques, MOMglmTest, omicsObj = omicsObj, classAnnot = classAnnot,
    ...
    names(alphas) <- NULL
    new(
        "MultiOmicsModules", alphas = alphas, zlists = zlists,
        modulesView = momics, modules = cliques, analysis = analysis,
