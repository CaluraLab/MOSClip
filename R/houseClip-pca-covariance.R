...
estimateExprCov <- function(expr, shrink) {
    if (shrink)
        unclass(corpcor::cov.shrink(expr, verbose = FALSE)) else cov(expr)
}
...
makePositiveDefinite <- function(m1, m2 = NULL, m3 = NULL, threshold = 0.1) {
    ...
    if (is.null(m2) &
        ...
        if (any(is.null(c(m2, m3))))
            stop("Both m2 and m3 are needed")
    ...
    if (!(eig > 0)) {
        ...
        m2n <- if (!is.null(m2))
            m2 + (threshold - eig) * diag(n) else NULL
        m3n <- if (!is.null(m3))
            m3 + (threshold - eig) * diag(n) else NULL
    ...
    return(
        list(
            m1 = m1n, m2 = m2n, m3 = m3n, correction = correction,
            value = value
        )
    )
}
