...
makeOmics <- function(experiments = ExperimentList(),
                      ...
                      modelInfo = character(), specificArgs = list()) {
    if (missing(sampleMap)) {
        ...
        colData <- S4Vectors::DataFrame(row.names <- unique(
            sampleMap[["primary"]]
        ))
    ...
    if (length(MAE@ExperimentList) != length(modelInfo)) {
        message("Data and relative methods to analyze ",
                "them must be equal in length")
        return()
    }
    ...
    if (any(match)) {
        message(paste(
            ...
            "modelInfo not found in method.",
            "Try availableOmicMethods."
        ))
        return()
    }
    ...
    data_validation <- lapply(MAE@ExperimentList@listData, function(exp) {
        all(exp == 0)
    })

    if (any(data_validation == TRUE)) {
        stop(paste0(
            "Some experiment matrices contain only 0s: ",
            paste(names(data_validation)[
                which(data_validation == TRUE)
            ], collapse = ", ")
        ))
    }
    ...
    if (any(data_validation == TRUE)) {
      stop(paste0(
        "Some experiment matrices contain NA values: ",
        paste(names(data_validation)[
          which(data_validation == TRUE)
        ], collapse = ", ")
      ))
    ...
    new("Omics",
        ExperimentList = MAE@ExperimentList, colData = MAE@colData,
        sampleMap = MAE@sampleMap, metadata = MAE@metadata,
        modelInfo = modelInfo, specificArgs = specificArgs
    )
}
...
setMethod("showOmics", signature(object = "Omics"), function(object) {
...
setMethod("showModule",
    ...
    definition = function(object) {
        ...
        if (length(sthis) < length(sigCliquesIdx)) {
            cat(paste0(
                "There are other ", length(sigCliquesIdx) - length(sthis),
