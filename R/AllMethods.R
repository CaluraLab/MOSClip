...
makeOmics <- function(
    experiments = ExperimentList(), colData = S4Vectors::DataFrame(),
    sampleMap = S4Vectors::DataFrame(assay = factor(), primary = character(),
                                     colname = character()),
    metadata = list(), drops = list(), modelInfo = character(),
    specificArgs = list()
) {
    ...
    if (any(match)) {
        message(
            paste(
                paste(modelInfo[match], collapse = ", "),
    ...
    if (length(unique(samplesNumber)) !=
        1) {
    ...
    duplo <- vapply(
        MAE@ExperimentList, function(data) {
    ...
    if (any(duplo)) {
        message(
            paste0(
                "Duplicated row.names found in omics ", paste(
                    which(duplo),
                    collapse = ", "
              )
            )
    ...
    if (any(data_validation == TRUE)) {
        stop(
            paste0(
                ...
                paste(
                    names(data_validation)[which(data_validation == TRUE)],
                    collapse = ", "
              )
            )
        )
    ...
    new(
        "Omics", ExperimentList = MAE@ExperimentList, colData = MAE@colData,
        sampleMap = MAE@sampleMap,
        metadata = MAE@metadata, modelInfo = modelInfo,
...
setMethod(
    "showOmics", signature(object = "Omics"),
    function(object) {
        ...
        for (i in seq_along(nm)) {
            cat(
                paste0(
                  "Data \"", nm[i], "\" to be processed with \"",
                  object@modelInfo[[i]], "\". "
              )
            )
            if (is.null(object@specificArgs[[i]])) {
                ...
                cat("Arguments:\n")
                arguments <- vapply(
                  seq_along(object@specificArgs[[i]]),
                  function(argI) {
                    ...
                    paste(
                      names(object@specificArgs[[i]])[argI],
                      values, sep = " :"
                  )
                  }, character(1)
              )
...
#' 
#' @export
setGeneric("showModule", function(object) standardGeneric("showModule"))
...
setMethod(
    "showModule", signature = "MultiOmicsModules",
    definition = function(object) {
        sthis <- seq_len(
            min(
                length(object@alphas),
                3
            )
        )
        ...
        for (i in sthis) {
            cat(
                paste0(
                  "Module ", i, ": pvalue ", object@alphas[i],
                  "\n"
              )
            )
            ...
            if (length(covs) !=
                0)
                cat(
                  "The following covariates are implicated:\n",
                  paste(covs, collapse = ", "),
                  "\n"
              )
        ...
        if (length(sthis) <
            length(sigCliquesIdx)) {
            cat(
                paste0(
                  "There are other ", length(sigCliquesIdx) -
                    length(sthis),
...
#' 
#' @export
setGeneric("showPathway", function(object) standardGeneric("showPathway"))
...
setMethod(
    "showPathway", signature = "MultiOmicsPathway",
    definition = function(object) {
        if (!is.null(object@title)) {
            cat("\"", object@title, "\"\n", sep = "")
        }
        cat(
            paste0(
                "Pathway overall pvalue: ", object@pvalue,
                "\n"
            )
        )
        invisible(NULL)
...
setGeneric(
    "convertPathway",
    function(graph, useTheseGenes) standardGeneric("convertPathway")
)
...
setMethod(
    "convertPathway", signature("Pathway", "characterOrNULL"),
...
setMethod(
    "convertPathway", signature("graphNEL", "characterOrNULL"),
    function(graph, useTheseGenes) {
        if (!is.null(useTheseGenes)) {
            usableGenes <- intersect(useTheseGenes, graph::nodes(graph))
            graph <- graph::subGraph(usableGenes, graph)
...
#' @importFrom graph subGraph
setMethod(
    "convertPathway", signature("character", "characterOrNULL"),
    function(graph, useTheseGenes) {
