...
multiPathwayReport <- function(multiPathwayList, priority_to = NULL) {
    if (!is(multiPathwayList, "list") || 
        any(vapply(multiPathwayList, 
                   class, character(1)) != "MultiOmicsPathway")) {
        stop("A list of pathway results are expected.")
    }

    pvalues <- vapply(multiPathwayList, function(p) {
        as.numeric(p@pvalue)
    }, numeric(1))
...
multiPathwayModuleReport <- function(multiPathwayModuleList, 
    ...
    if (!is(multiPathwayModuleList, "list") || 
        any(vapply(multiPathwayModuleList, 
                   class, character(1)) != "MultiOmicsModules")) {
    ...
    multiMatrixRes <- lapply(n_temp, function(name, list) {
        ...
        data.frame(
            pathway = name, module = row.names(summary), 
            summary, row.names = NULL, stringsAsFactors = FALSE)}, 
        multiPathwayModuleList)
    resDF <- mergeAll(multiMatrixRes)
    resDF <- resDF[order(resDF$pvalue), ]
    rownames(resDF) <- apply(resDF, 1, function(r) {
        paste(r["pathway"], r["module"], sep = ".")
    })
...
mergeAll <- function(list) {
    ...
    matrix <- do.call(rbind, lapply(list, function(o) {
...
order_by_covariates <- function(dataF, skip_first_cols, priority_to = NULL) {
    ...
    covariates <- dataF[, seq_len(ncol(dataF) - skip_first_cols) 
