...
multiPathwayReport <- function(multiPathwayList, priority_to = NULL) {
    if (!is(multiPathwayList, "list") ||
        any(
            vapply(multiPathwayList, class, character(1)) !=
                "MultiOmicsPathway"
        ))
        stop("A list of pathway results are expected.")

    pvalues <- vapply(
        multiPathwayList, function(p) {
            as.numeric(p@pvalue)
        }, numeric(1)
    )
    ...
    ord <- order(pvalues)
    df <- cbind(
        row.names = names(pvalues)[ord],
        pvalue = pvalues[ord], data.frame(zMat[ord, , drop = FALSE])
    )
...
multiPathwayModuleReport <- function(
    ...
    multiMatrixRes <- lapply(
        n_temp, function(name, list) {
            summary <- formatModuleReport(list[[name]])
            data.frame(
                pathway = name,
                module = row.names(summary),
                summary, row.names = NULL,
    ...
    rownames(resDF) <- apply(
        resDF, 1, function(r) paste(
            r["pathway"], r["module"],
            sep = "."
        )
    )
...
mergeAll <- function(list) {
    ...
    removeCols <- match(
        c("pathway", "module", "pvalue"),
        colnames(matrix)
    )
    numericMat <- matrix[, -removeCols, drop = FALSE]
    data.frame(
        pathway = matrix[, "pathway"], module = matrix[, "module"],
        pvalue = as.numeric(matrix[, "pvalue"]),
        apply(numericMat, 2, as.numeric),
        stringsAsFactors = FALSE
    )
}
...
order_by_covariates <- function(dataF, skip_first_cols, priority_to = NULL) {
    ...
    covariates <- dataF[, seq_len(
        ncol(dataF) -
            skip_first_cols
    ) +
        skip_first_cols, drop = FALSE]
    fixed <- dataF[, seq_len(skip_first_cols),
        drop = FALSE]
    omics_cov <- guessOmics(colnames(covariates))
    to_sort <- unique(omics_cov) %in%
        priority_to
    ...
    covariates <- covariates[, order(cov_factor),
        drop = FALSE]
