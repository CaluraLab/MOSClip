...
extractSummaryFromBinary <- function(omic, multiOmicObj, n = 3) {
    ...
    impact <- lapply(
        covs, mostlyMutated, moduleMat = t(moduleMat),
    ...
    discrete <- data.frame(
        lapply(omic$x, as.numeric),
        row.names = row.names(omic$x)
    )
    list(
        sigModule = sigModule, discrete = discrete, subset = mostlyImpacted,
...
extractSummaryFromCluster <- function(omic, multiOmicObj, n = 3) {
    ...
    sigModule <- moduleMat[row.names(involved),
        , drop = FALSE]
    topGenes <- genes[row.names(involved)]
    topGenes <- tapply(
        names(topGenes),
        topGenes, paste, collapse = ";"
    )

    list(
        sigModule = sigModule, discrete = omic$x, subset = data.frame(
            row.names = names(topGenes),
            metClust = topGenes
        ),
        pvalues = involved, covsConsidered = omic$namesCov
    )
}
...
#' \item{covsConsidered}{the name of the considered omic}
extractSummaryFromPCA <- function(
    omic, multiOmicObj, moduleCox, analysis,
    loadThr = 0.6, atleast = 1, minprop = 0.1
) {
    covs <- omic$namesCov
    lds <- omic$loadings
    discretePC <- createDiscreteClasses(
        coxObj = moduleCox, covs, analysis,
        minprop = minprop
    )
    topLoad <- extractHighLoadingsGenes(lds, thr = loadThr, atleast = atleast)
    moduleMat <- createDataModule(omic, multiOmicObj)
    sigModule <- moduleMat[row.names(topLoad),
        , drop = FALSE]

    list(
        sigModule = sigModule, discrete = discretePC,
        subset = topLoad, covsConsidered = covs
    )
}

#' @importFrom survminer surv_cutpoint surv_categorize
#' @importFrom stats median
createDiscreteClasses <- function(
    coxObj, covs, analysis, labels = c("low", "high"),
    minprop = 0.1
) {
    ...
    check <- vapply(
        coxObj[, covs, drop = FALSE], check_minimal_proportion,
        c(min_prop = minprop),
        logical(1)
    )
    if (any(!check)) {
    ...
    if (analysis == "survival") {
        sc <- surv_cutpoint(
            coxObj, time = "days", event = "status", variables = covs,
            minprop = minprop
        )
        surv_categorize(sc, labels = labels)
    } else if (analysis == "twoClass") {
        covs_classified <- lapply(
            covs, function(cov) {
                median_value <- median(
                  as.numeric(coxObj[[cov]]),
                  na.rm = TRUE
              )
...
extractHighLoadingsGenes <- function(loadings, thr, atleast = 1) {
    l <- lapply(
        ...
        function(pc) {
            ...
            data.frame(
                row.names = genes, component = rep(pc, length(genes)),
...
collapse <- function(list) {
    df <- data.frame(
        genes = unlist(lapply(list, function(x) row.names(x))),
        components = unlist(lapply(list, function(x) x$component)),
        stringsAsFactors = FALSE
    )
    tapply(
        seq_len(NROW(df)),
        df$genes, function(idx) {
...
#' \item{covsConsidered}{the name of the considered omic}
extractSummaryFromNumberOfEvents <- function(
    omic, multiOmicObj, moduleCox, analysis, n = 3, minprop = 0.1,
    labels = c("few", "many")
) {
    covs <- omic$namesCov
    moduleMat <- createDataModule(omic, multiOmicObj)
    discreteClass <- createDiscreteClasses(
        coxObj = moduleCox, covs, analysis, labels = labels,
        minprop = minprop
    )
    ...
    list(
        sigModule = sigModule, discrete = discreteClass, subset = involved,
...
mostlyMutated <- function(cov, moduleMat, name, eventThr = 2) {
    ...
    priority <- data.frame(
        row.names = names(priority),
        priority
    )
    names(priority) <- cov
    priority[order(priority[[cov]], decreasing = TRUE),
        , drop = FALSE]
}
