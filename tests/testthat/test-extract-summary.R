test_that("extractyInfo_binary", {
  ...
  mo <- makeOmics(experiments = ExperimentList(mut=mut, mut2=mut), 
                  colData = dummy_colData("survival"), 
                  modelInfo = c("summarizeToBinaryEvents",
...
test_that("extractyInfo_dirBinary", {
  ...
  omic <- summarizeToBinaryDirectionalEvents(fake_cnv, row.names(fake_cnv),
                                             name="cnv")
  mo <- makeOmics(experiments = ExperimentList(cnv=fake_cnv,
                                               mut=dummy_mut()), 
                  colData = dummy_colData("survival"), 
                  modelInfo = c("summarizeToBinaryDirectionalEvents",
                                "summarizeToNumberOfEvents"),
                  specificArgs = list(countEvent=list(name="cnv"),
                                      countEvent2=list(name="mut")))
