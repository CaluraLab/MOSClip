...
dummy_cnv_like_dataset <- function(genes = NULL, seed = 1234) {
    ...
    lowValues <- sample(c(-1, 0, 0, 0, 1, 1), patientsRatio[[1]][1] *
        genesSize[1], replace = TRUE)
    highValues <- sample(c(-2, 0, 0, 0, 0, 2, 2), patientsRatio[[1]][2] *
        genesSize[1], replace = TRUE)
    lowValues2nd <- sample(c(-2, 0, 0, 0, 0, 0, 2), patientsRatio[[2]][1] *
        genesSize[2], replace = TRUE)
    highValues2nd <- sample(c(-1, 0, 0, 0, 0, 0, 1), patientsRatio[[2]][2] *
        genesSize[2], replace = TRUE)

    fake_cnv <- rbind(cbind(matrix(lowValues, ncol = patientsRatio[[1]][1],
        nrow = genesSize[1]), matrix(highValues, ncol = patientsRatio[[1]][2],
        nrow = genesSize[1])), cbind(matrix(lowValues2nd,
        ncol = patientsRatio[[2]][1], nrow = genesSize[2]),
...
dummy_methylation_like_dataset <- function(genes = NULL, seed = 1234) {
    ...
    lowValues <- runif(patientsRatio[[1]][1] * genesSize[1],
        0, 0.2)
    highValues <- runif(patientsRatio[[1]][2] * genesSize[1],
        0.7, 0.99)
    lowValues2nd <- runif(patientsRatio[[2]][1] * genesSize[2],
        0.6, 1)
    highValues2nd <- runif(patientsRatio[[2]][2] * genesSize[2],
        0.2, 0.4)
...
dummy_methylation_like_flat_dataset <- function(genes = NULL, seed = 1234) {
    flat_data <- matrix(c(rep(0.3, 191), rep(0.4, 9)), ncol = 200,
        nrow = 1, dimnames = list(paste0("gene_", seq_len(1)), paste0("P_",
            seq_len(200))))
...
dummy_expression_like_dataset <- function(genes = NULL, seed = 1234) {
    ...
    lowValues <- rnorm(patientsRatio[[1]][1] * genesSize[1],
        5, 1)
    highValues <- rnorm(patientsRatio[[1]][2] * genesSize[1],
        8, 1)
    lowValues2nd <- rnorm(patientsRatio[[2]][1] * genesSize[2],
        9, 1)
    highValues2nd <- rnorm(patientsRatio[[2]][2] * genesSize[2],
        2, 1)
...
dummy_colData <- function(len = 200, type = "survival") {
    ...
    num2 <- len - num1
    if (type == "two-classes") {
        dummy_colData <- data.frame(classes = c(rep("A", num1), rep("B",
            num2)))
    } else if (type == "survival") {
        dummy_colData <- data.frame(status = sample(c(0, 1), size = len,
            replace = TRUE), days = sample(5:5000, size = len, replace = TRUE))
    }
...
fake_mo <- function(genes = NULL, type = "survival",
    ...
    exp_list <- ExperimentList(exp = exp, mut = mut,
        met = met, cnv = cnv)
    cdata <- dummy_colData(len = dim(exp)[2],
        type)
    modelInfo = c(exp = "summarizeWithPca",
        met = "summarizeInCluster", mut = "summarizeToNumberOfEvents",
        cnv = "summarizeToNumberOfDirectionalEvents")
    specificArgs = list(exp = list(name = "exp"),
                        met = list(name = "met"), mut = list(name = "mut"),
                        cnv = list(name = "cnv"))
    mo <- makeOmics(experiments = exp_list[omics],
        colData = cdata, modelInfo = modelInfo[omics],
