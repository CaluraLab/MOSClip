...
test_that("computePCs", {
  ...
  pcs <- summarizeWithPca(fake_exp, features=row.names(fake_exp), name="pca", shrink=FALSE, method="regular", 
                          cliques=NULL, maxPCs=10, loadThr=0.6)
  pcsS <- summarizeWithPca(fake_exp, features=row.names(fake_exp), name="pca", shrink=FALSE, method="sparse", 
                           cliques=NULL, maxPCs=10, loadThr=0.6)
  pcsT <- summarizeWithPca(fake_exp, features=row.names(fake_exp), name="pca", shrink=FALSE, method="topological",
                           cliques=list(row.names(fake_exp), row.names(fake_exp)[5]), maxPCs=10, loadThr=0.6)
